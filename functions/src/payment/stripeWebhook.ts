...
import { splitConnect, stripSnapshotMoney } from './orderMoney';
  ...
  async (request, response) => {
    try {
      ...
      if (event.type === 'payment_intent.succeeded') {
        ...
        let fullSnapshot: ProductionSnapshot | null = null;
        try {
          ...
          if (metadata.productionSnapshotRequired === 'false') {
            ...
            Object.assign(orderData, {
              ...
              productionSnapshot: stripSnapshotMoney(productionSnapshot),
            });
          }
        ...
        // order anyway).
        try {
          await db.collection('checkouts').doc(paymentIntent.id).set(
            { status: 'completed', completedAt: new Date() },
            { merge: true }
          );
        } catch (recoveryError: any) {
          logger.warn('⚠️ checkout-recovery: failed to mark checkout completed', {
            paymentIntentId: paymentIntent.id,
        ...
        if (failedPi.metadata?.source === 'b2c_shop') {
          try {
            await db.collection('checkouts').doc(failedPi.id).set(
              { status: 'failed', failedAt: new Date() },
              { merge: true }
            );
          } catch (recoveryError: any) {
            logger.warn('⚠️ checkout-recovery: failed to mark checkout failed', {
