...
const hostVerdicts = new Map<string, { safe: boolean; at: number }>(); // hostname → resolved verdict
...
export async function assertPublicUrl(raw: string): Promise<URL> {
  ...
  if (isBlockedHost(host)) throw new Error('URL points to a blocked host');
  if (!isIP(host)) {
    const cached = hostVerdicts.get(host);
    let safe = cached && Date.now() - cached.at < VERDICT_TTL_MS ? cached.safe : undefined;
    if (safe === undefined) {
      const addrs = await lookup(host, { all: true, verbatim: true });
      safe = addrs.length > 0 && addrs.every((r) => !isBlockedHost(r.address));
      hostVerdicts.set(host, { safe, at: Date.now() });
    }
    if (!safe) throw new Error('URL resolves to a private address');
  }
  return u;
}
...
export async function readBodyCapped(res: Response, maxBytes: number): Promise<Buffer> {
  ...
  if (!reader) return Buffer.from(await res.arrayBuffer()); // no stream → header cap already applied
...
export async function safePublicFetch(
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    if (opts.maxBytes && len > opts.maxBytes) throw new Error('Response too large');
    return res;
  }
