...
async function computeOrderTotalsSek(cartItems, shippingCountry, discountCode, shopId, deliveryMethod) {
    ...
    if (discountCode && discountSource === null && await (0, shopFeatures_1.isShopFeatureEnabled)(shopId, 'discountCodes')) {
        const dcSnap = await database_1.db.collection('discountCodes')
            .where('shopId', '==', shopId)
        ...
        if (!dcSnap.empty) {
            ...
            if (windowOk && usesOk && minSpendOk) {
                ...
                const productIds = Array.isArray(dc.productIds) ? dc.productIds : [];
                const base = scope === 'products'
                    ? loaded.reduce((sum, { productId, price, quantity }) => sum + (productIds.includes(productId) ? price * quantity : 0), 0)
                    : subtotal;
...
exports.createPaymentIntentV2 = (0, https_1.onRequest)({
    ...
    try {
        ...
        if (podEnabled) {
            try {
                ...
                if (unresolvedLines.length > 0) {
                    ...
                    const slotNotPrintable = unresolvedLines.some((line) => line.unresolvedReason === printProjection_1.SLOT_NOT_PRINTABLE_REASON);
        ...
        if (podEnabled && withholding.unrouted.length > 0) {
            firebase_functions_1.logger.error('⛔ Checkout blocked — POD item routed to no printer', {
                shopId: resolvedShopId,
