...
exports.refundOrder = (0, https_1.onCall)({
    ...
    const validation = (0, connectParams_1.validateRefundRequest)(chargedSek, refundedBeforeSek, requestedAmount);
    if (!validation.ok) {
        throw new https_1.HttpsError('invalid-argument', validation.error || 'Invalid refund amount');
    }
    ...
    const idempotencyKey = `refund:${orderId}:${paymentIntentId}:${refundedBeforeSek.toFixed(2)}:${requestedAmount ?? 'rest'}`;
    ...
    const state = (0, connectParams_1.refundStateAfter)(chargedSek, refundedBeforeSek, refundedNowSek);
    const patch = {
        status: state.status,
        updatedAt: firestore_1.FieldValue.serverTimestamp(),
        'payment.refundId': refund.id,
        'payment.refundIds': firestore_1.FieldValue.arrayUnion(refund.id),
