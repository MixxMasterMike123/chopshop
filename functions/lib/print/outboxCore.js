...
exports.isClaimablePrintNotification = exports.retryDelayMs = exports.DELIVERY_LEASE_MS = exports.INITIAL_SWEEP_DELAY_MS = exports.MAX_ATTEMPTS = exports.shouldEnqueuePrintNotify = exports.isPaidLifecycleStatus = void 0;
...
exports.isPaidLifecycleStatus = isPaidLifecycleStatus;
/**
 ...
 */
function shouldEnqueuePrintNotify(beforeStatus, afterStatus) {
    if (!afterStatus || !NOTIFY_STATUSES.has(afterStatus))
        return false;
    if (beforeStatus == null)
        return true; // creation
    return !PAID_LIFECYCLE.has(beforeStatus);
}
exports.shouldEnqueuePrintNotify = shouldEnqueuePrintNotify;
...
exports.INITIAL_SWEEP_DELAY_MS = 5 * 60 * 1000;
...
exports.DELIVERY_LEASE_MS = 5 * 60 * 1000;
 ...
 */
function isClaimablePrintNotification(input) {
    if (input.status !== 'pending')
        return false;
    if ((Number(input.attempts) || 0) >= exports.MAX_ATTEMPTS)
        return false;
    if (!input.ignoreSchedule && Number(input.nextAttemptAtMs || 0) > input.nowMs)
        return false;
    if (Number(input.leaseUntilMs || 0) > input.nowMs)
        return false;
    return true;
}
exports.isClaimablePrintNotification = isClaimablePrintNotification;
//# sourceMappingURL=outboxCore.js.map