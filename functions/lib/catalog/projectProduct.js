...
exports.projectPublicProduct = void 0;
...
const GROUP_KEYS = ['label', 'sku', 'price', 'image', 'images', 'sizes'];
/** Legacy v2.1 options-matrix rows (cleared to [] on re-save, still rendered). */
const OPTION_KEYS = ['name', 'values'];
 ...
 */
function projectPublicProduct(raw) {
    if (!raw)
        return null;
    ...
    if (raw.isActive !== true || availability?.b2c !== true)
        return null;
    ...
    const pickRows = (rows, keys) => Array.isArray(rows)
        ? rows.filter((r) => !!r && typeof r === 'object').map((r) => pick(r, keys))
        : undefined;
    ...
    if (variantGroups)
        pub.variantGroups = variantGroups;
    const options = pickRows(raw.options, OPTION_KEYS);
    if (options)
        pub.options = options;
    return pub;
}
exports.projectPublicProduct = projectPublicProduct;
//# sourceMappingURL=projectProduct.js.map