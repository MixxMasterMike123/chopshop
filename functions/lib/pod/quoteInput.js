...
exports.parseQuoteInput = exports.MAX_QUOTE_SLOTS = exports.QUOTE_SLOTS = void 0;
/** The POD placement-slot vocabulary (printProjection.ts PlacementSlot). */
exports.QUOTE_SLOTS = ['front', 'back', 'pocket', 'left_sleeve', 'right_sleeve', 'other'];
exports.MAX_QUOTE_SLOTS = 6;
/**
 ...
 */
function parseQuoteInput(data) {
    const d = (data && typeof data === 'object' ? data : {});
    const shopId = typeof d.shopId === 'string' ? d.shopId.trim() : '';
    if (!shopId)
        return 'shopId saknas';
    const garment = typeof d.garment === 'string' ? d.garment.trim() : '';
    if (!garment || garment.length > 40)
        return 'garment saknas eller är ogiltigt';
    if (!Array.isArray(d.slots) || d.slots.length > exports.MAX_QUOTE_SLOTS) {
        return `slots måste vara en lista med högst ${exports.MAX_QUOTE_SLOTS} ytor`;
    }
    const known = new Set(exports.QUOTE_SLOTS);
    if (!d.slots.every((s) => typeof s === 'string' && known.has(s)))
        return 'okänd tryckyta i slots';
    return { shopId, garment, slots: [...new Set(d.slots)] };
}
exports.parseQuoteInput = parseQuoteInput;
//# sourceMappingURL=quoteInput.js.map