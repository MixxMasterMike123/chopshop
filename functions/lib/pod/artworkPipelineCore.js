...
exports.runArtworkPipeline = exports.PIPELINE_VERSION = void 0;
// artworkPipelineCore — the PURE compute half of the POD artwork pipeline.
//
// Extracted from processArtwork.ts (2026-08-22, checkpoint 26) so the same
// battle-tested sharp pipeline serves BOTH callers byte-for-byte:
//   (1) the live `processPodArtwork` onCall (Firebase Storage in/out), and
//   (2) the dark render-farm job endpoint (`src/render-farm/`), which is what a
//       future Cloudflare Container would reimplement against this same shape.
// Same extraction discipline as `email-orchestrator/functions/migrationShared.ts`:
// the gate math, thresholds, Swedish user-facing messages, notice codes, decode/
// trim/ICC/encode ordering and the sharp memory settings were MOVED VERBATIM.
// Do NOT "improve" any of it — this is the 300-DPI gate the print shop contract
// (docs/POD_PRINT_SPEC.md, locked 2026-07-27) is written against, and the client
// mirror (src/utils/podValidation.js) is kept in sync with it by hand.
//
// PURITY CONTRACT (enforced by review, and by the verification script that
// imports this module from a bare node process):
//   - NO firebase-admin, NO Storage, NO Firestore, NO auth, NO HttpsError.
//   - Input  = the original file's bytes + the five profile fields.
//   - Output = a rejection, or the two output BUFFERS plus the measured facts.
//   - Nothing here knows about shops, tenants, storage paths, tokens or URLs.
//     Those belong to whichever wrapper is calling (see PipelineOk.meta).
//
// What stayed in the WRAPPER (io-bound by nature, unchanged from before):
//   storage existence check, download, the `randomUUID` id/token minting, the
//   `pod-artwork/{shopId}/...` storage paths, the uploads, `tokenUrl`, the
//   Firestore doc-field assembly (status/printUrl/.../validation) and orphan
//   cleanup. `checkedAt` is stamped by the wrapper at doc-assembly time exactly
//   as before — a wall-clock read is not part of a pure pipeline.
...
// MOVED VERBATIM from processArtwork.ts. These are process-global sharp settings
// and stay a module-load side effect so BOTH callers inherit them by importing
// the core — the callable no longer sets them itself.
//
// ⚠️ MAINTAINER NOTE: this module is currently the ONLY sharp importer in
// functions/src, which is what makes the import side effect airtight. If you add
// another sharp user, import it THROUGH this module or repeat these two lines —
// and never do sharp WORK at module scope, because that is the one shape that
// could run before these settings land. Cold-start ordering is otherwise
// harmless: every export is loaded long before the first request.
...
const FORMAT_TO_EXT = { jpeg: 'jpg', png: 'png', tiff: 'tiff', webp: 'webp' };
...
const looksLikeHeic = (buf) => buf.length > 12 && buf.subarray(4, 8).toString('ascii') === 'ftyp' &&
 ...
 */
async function runArtworkPipeline(buf, profile) {
    const reasons = [];
    const notices = [];
    // ---- size gate ----
    ...
    let meta;
    try {
        meta = await (0, sharp_1.default)(buf, { limitInputPixels: false }).metadata();
    }
    catch (e) {
        if (looksLikeHeic(buf))
            return { ok: false, reasons: [{ code: 'format_heic', message: SPECIAL_FORMAT_FAIL.heif }] };
        return { ok: false, reasons: [{ code: 'unreadable', message: 'Filen kunde inte läsas — den kan vara skadad. Ladda ner originalet igen och försök på nytt.' }] };
    }
    const fmt = meta.format || '';
    if (SPECIAL_FORMAT_FAIL[fmt]) {
        return { ok: false, reasons: [{ code: `format_${fmt}`, message: SPECIAL_FORMAT_FAIL[fmt] }] };
    }
    ...
    if (!ext || !accepted.includes(ext)) {
        ...
        return { ok: false, reasons: [{ code: 'format_not_accepted', message: `Formatet ${fmt ? '.' + fmt : ''} stöds inte. Tillåtna format: ${allowed}.` }] };
    }
    if (!meta.width || !meta.height) {
        return { ok: false, reasons: [{ code: 'unreadable', message: 'Kunde inte läsa bildens mått — filen kan vara skadad.' }] };
    }
    ...
    if (Math.max(srcW, srcH) > MAX_SOURCE_PX) {
        return { ok: false, reasons: [{ code: 'px_too_large', message: `Bilden är ${srcW} × ${srcH} px — max ${MAX_SOURCE_PX} px på längsta sidan. Skala ner filen.` }] };
    }
    ...
    let work = (0, sharp_1.default)(buf, { limitInputPixels: (MAX_SOURCE_PX + 2000) * (MAX_SOURCE_PX + 2000) }).rotate();
    ...
    if (meta.hasAlpha) {
        ...
        try {
            ...
            png = await (0, sharp_1.default)(buf, { limitInputPixels: (MAX_SOURCE_PX + 2000) * (MAX_SOURCE_PX + 2000) })
                .rotate()
                .toColourspace('srgb')
                .png()
                .toBuffer();
    ...
    const outMeta = await (0, sharp_1.default)(png).metadata();
    const width = outMeta.width || 0;
    const height = outMeta.height || 0;
    if (!width || !height) {
        return { ok: false, reasons: [{ code: 'unreadable', message: 'Bearbetningen gav en tom bild — filen kan vara skadad.' }] };
    }
    ...
    if (effectiveDpi < profile.min_dpi) {
        reasons.push({
            ...
            message: `Motivet är ${width} × ${height} px. I sin största tryckstorlek ` +
    ...
    if (alphaStats.visibleRatio === 0 && alphaStats.hasAlphaChannel) {
        ...
        return { ok: false, reasons: [{ code: 'fully_transparent', message: 'Bilden är helt genomskinlig — det finns inget motiv att trycka.' }] };
    }
    ...
    // ---- render the preview WebP ----
    // The wrapper (or the farm) decides WHERE these two buffers go; the core only
    // produces them. In the callable this was interleaved with the uploads.
    const preview = await (0, sharp_1.default)(png)
...
exports.runArtworkPipeline = runArtworkPipeline;
...
async function alphaProfile(png) {
...
//# sourceMappingURL=artworkPipelineCore.js.map