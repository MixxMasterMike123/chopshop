...
exports.renderFarmProcessArtwork = void 0;
...
const artworkPipelineCore_1 = require("../pod/artworkPipelineCore");
...
// SSRF one.
const ALLOWED_INPUT_HOST_SUFFIXES = ['.r2.cloudflarestorage.com'];
 ...
 */
const bearerMatches = (presented, expected) => {
    const a = (0, crypto_1.createHash)('sha256').update(presented, 'utf8').digest();
    const b = (0, crypto_1.createHash)('sha256').update(expected, 'utf8').digest();
...
const isValidProfile = (p) => {
    ...
    if (!isNonEmptyString(p.id))
        return false;
    if (!isPositiveSafeInt(p.min_dpi))
        return false;
    const area = p.print_area_mm;
    if (!area || typeof area !== 'object')
        return false;
    if (typeof area.w !== 'number' || !Number.isFinite(area.w) || area.w <= 0)
        return false;
    if (typeof area.h !== 'number' || !Number.isFinite(area.h) || area.h <= 0)
        return false;
    // max_file_mb is optional in the core (0/absent disables the size gate), but
    // when present it must be a real positive number.
    if (p.max_file_mb !== undefined && (typeof p.max_file_mb !== 'number' || !Number.isFinite(p.max_file_mb) || p.max_file_mb <= 0))
        return false;
    ...
    if (p.accepted_formats !== undefined) {
        if (!Array.isArray(p.accepted_formats))
            return false;
        if (!p.accepted_formats.every((f) => f && typeof f === 'object' && isNonEmptyString(f.ext)))
            return false;
    }
    return true;
};
const isValidEnvelope = (body) => {
    ...
    if (body.contract !== 1)
        return false;
    if (body.jobType !== 'pod.process_artwork')
        return false;
    if (!isNonEmptyString(body.jobId))
        return false;
    if (!body.input || typeof body.input !== 'object')
        return false;
    if (!isString(body.input.url))
        return false;
    if (!isPositiveSafeInt(body.input.maxBytes))
        return false;
    if (!isValidProfile(body.profile))
        return false;
    if (!body.output || typeof body.output !== 'object')
        return false;
    if (!isString(body.output.printPngPutUrl))
        return false;
    if (!isString(body.output.previewWebpPutUrl))
        return false;
    return true;
};
/** https + hostname suffix allowlist. Returns the parsed URL or null. */
const parseAllowedUrl = (raw) => {
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (!ALLOWED_INPUT_HOST_SUFFIXES.some((suffix) => host.endsWith(suffix)))
        return null;
    return url;
};
/**
...
async function fetchInput(url, maxBytes) {
    ...
    if (!res.ok || !res.body)
        throw new Error(`input_fetch_status_${res.status}`);
...
async function putOutput(url, body, contentType) {
    ...
    if (!res.ok)
        throw new Error(`output_put_status_${res.status}`);
}
const sha256Hex = (buf) => (0, crypto_1.createHash)('sha256').update(buf).digest('hex');
exports.renderFarmProcessArtwork = (0, https_1.onRequest)(OPTS, async (req, res) => {
    ...
    let result;
    try {
        result = await (0, artworkPipelineCore_1.runArtworkPipeline)(buf, profile);
    }
    catch (err) {
...
//# sourceMappingURL=processArtworkJob.js.map