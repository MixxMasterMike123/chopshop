...
**Last updated:** 2026-08-22 late, checkpoint 25 LIVE E2E VERIFIED (full money loop + replay safety against real Stripe); checkpoint 27 deployed dark — Fable orchestrating, Opus building, Fable reviewing
...
- Runtime: one modular API Worker, queue/scheduled jobs, static web deployment. ~~and a Container for Sharp/FFmpeg~~ — **superseded 2026-08-22 by the HYBRID decision below.**
- **HYBRID END-STATE (owner decision 2026-08-22, supersedes all-CF):** Cloudflare is the system of record and the ENTIRE synchronous HTTP surface (storefront, admin, auth, catalogue, checkout, payments, orders). Firebase is demoted to a stateless **render farm** for heavy media compute only — sharp (`processArtwork`) now, ffmpeg (content studio) later — reached exclusively through the job contract in `docs/RENDER_FARM_CONTRACT.md`: self-contained jobs, R2 signed URLs in/out, shared-secret auth, no Firestore/Firebase-Auth in the job path, farm never reads CF data. Rationale: sharp lives in exactly ONE file (`functions/src/pod/processArtwork.ts` — verified 2026-08-22); the print portal/projection/outbox code is queries + signed URLs and ports to Workers normally. This deletes the Containers unknown from the cutover path, preserves the battle-tested 300-DPI pipeline unchanged, and shrinks the remaining port to ~10–14 mechanical checkpoints. **Demolition principle:** every checkpoint from here on ends with the superseded Firebase functions DELETED — 75 exports shrink toward ~4–6 render functions. Per-tenant splitting (old shops Firebase / new shops CF) is allowed ONLY as the dual-run transition, never as steady state. The same contract could later be served by a CF Container if the farm is ever retired — a later decision from strength, not a bet now.
- **Revised remaining roadmap under hybrid (Minimum Cutover Surface):** (a) keystone: extract `runPipeline` pure core + dark `onRequest` job endpoint on Firebase (byte-identical shared module — the `migrationShared.ts` pattern; live app unchanged); (b) CF POD domain: profiles, artwork upload + job dispatch + verdict persistence, print portal reads, production snapshot, print outbox → Queue; (c) order surfaces (admin list/detail, buyer lookup); (d) transactional email via Resend HTTP from the Worker (Email Service canary stays optional); (e) refunds + Connect destination-charge seam; (f) storefront/admin hosting on CF; (g) pilot-tenant data migration + dual-run cutover. DEFERRED past cutover (stay on Firebase meanwhile or port later): DAC7, Woo/Shopify migrators, content studio, abandoned cart, reviews sweeps. Cadence: paced weekly sessions (Opus builds 2–3 checkpoints, Fable reviews as one batch), capped ~half the weekly Fable budget so live-business work never starves.
...
## Checkpoint 25 — Stripe webhook → durable orders → discount burn (✅ LIVE E2E VERIFIED 2026-08-22)

- **✅ FULL MONEY LOOP PROVEN LIVE 2026-08-22 late evening** (owner + Fable): anonymous checkout with SMOKE10 discount → server-minted PI → real test-card confirm → Stripe delivered `payment_intent.succeeded` (200, Levererad) → order `20260822-S2K41A2E` created with EXACT money (19900 − 1990 = 17910, VAT 3582, captured 17910), checkout `completed`, 1 frozen order line, birth transition NULL→paid, SMOKE10 `used_count` burned EXACTLY once, ledger `processed`. **Dashboard resend replayed the same event → 200 and ZERO change** (still 1 order / 1 ledger row / used_count 1) — replay safety proven against real Stripe. Operator script: `cloudflare/scripts/stg-payment-e2e.sh` (prompts for sk_test locally).
- **⚠️ STRIPE ENVIRONMENT LESSON (cost an hour of debugging):** the staging `STRIPE_SECRET_KEY` belongs to the **SANDBOX "Meteor Public Relations AB-sandlåda"** (`acct_1Tp7gtKAaBMOW5AC`) — NOT the account's classic "Test mode", and not Mohlen Media. Sandboxes and test mode are fully isolated environments with separate keys, events, and webhook destinations; the Workbench badge (with/without "-sandlåda") is the reliable tell, the dark "Sandbox" banner is NOT (it shows for test mode too). The webhook destination is `captivating-celebration` (`we_1U7LyAKAaBMOW5ACdF27I05U`) in that sandbox: correct URL, `payment_intent.succeeded` only, API version `2026-07-29.dahlia`, and `STRIPE_WEBHOOK_SECRET` is ITS signing secret. Any future Stripe config for staging goes in THAT sandbox.
